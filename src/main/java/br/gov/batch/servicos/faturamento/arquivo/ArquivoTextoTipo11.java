...
public class ArquivoTextoTipo11 extends ArquivoTexto {
	...
	private String getModuloDigitoVerificador(Short numeroModuloDigitoVerificador) {
		if (numeroModuloDigitoVerificador != null 
				&& numeroModuloDigitoVerificador.intValue() == ConstantesSistema.MODULO_VERIFICADOR_11.intValue()) {
	...
	private String getIndicadorSequencialLeitura(Rota rota) {
		return (rota != null && rota.getIndicadorSequencialLeitura() != null) ? Utilitarios.completaComZerosEsquerda(1,
				rota.getIndicadorSequencialLeitura()) : Utilitarios.completaComZerosEsquerda(1, Status.INATIVO.getId());
	}

	private Rota verificarRota(Imovel imovel) {
		return (imovel != null && imovel.getRotaAlternativa() != null) ? imovel.getRotaAlternativa() : 
			(imovel != null && imovel.getQuadra() != null) ? imovel.getQuadra().getRota() : null;
	}
